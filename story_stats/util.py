...
def split_text(content: str, max_content_len: int):
    ...
    numb_to_split = math.ceil(content_len / max_content_len)

    paragraph_list = content.strip().split('\n')
    numb_paragraphs = len(paragraph_list)

    paragraphs_per = int(numb_paragraphs / numb_to_split) + 1
    content_list = []
    for i in range(numb_to_split):
...
def remove_punct(text: str) -> str:
    ...
    return ONLY_LETTERS_SPACES.sub('', text)
...
def find_outliers(numb_list) -> List[int]:
    try:
        z_score_list = stats.zscore(numb_list)
...
def get_sample_word_lists(
    ...
    num_samples = int(num_words / min_sample_len)
    if num_samples == 0:
        num_samples = 1
